...
"""the_seeker: Stateless Windows 11 desktop agent with a visual memory HUD.

This program runs a vision-language model in a strictly screenshot-driven (stateless) loop:
- Input to the model is ONLY the current screenshot.
- The model MUST respond with EXACTLY ONE tool call per step.
- The cyan window titled "FRANZ MEMORY" is the agent's only memory because it is visible in screenshots.
  The program writes the model's memory text into the window, but does not read it back as model input.

HUD includes:
- PAUSE/RESUME (gates the loop; memory is editable while paused)
- Opacity slider (layered window alpha)
- A 3-state MODE box (Terminator-style gating):
    RED   = scan for an instruction/task
    GREEN = target check (validate quickly, then leave immediately)
    BLUE  = execute the task using mouse/keyboard tools

Windows 11 only. Python 3.12. Uses Win32 APIs via ctypes and RichEdit (RICHEDIT50W).
"""
...
HUD_SIZE = 1
# Default RichEdit font height (negative height in CreateFontW). 16 is typical; 48 ~= 3x.
HUD_FONT_HEIGHT = 16

# MODE box states (color-coded in the HUD): red=scan, green=validate, blue=execute.
MODE_SCAN = 0
MODE_VALIDATE = 1
MODE_TASK = 2

# If the MODE box stays GREEN too long (model got stuck), auto-return to RED.
VALIDATE_TIMEOUT_SEC = 100.0
...
SYSTEM_PROMPT = """You are FRANZ, a small Windows screen agent.

Input: ONLY the screenshot.
Output: EXACTLY ONE tool call.

Memory:
The cyan window titled "FRANZ MEMORY" is the only memory. Its text may be 1 frame old.
Write memory as overlapping time-windows:
Recently: ... / Now: ... / Soon: ...
Never write about the MODE box, colors, or modes.

MODE box (bottom of FRANZ MEMORY):
RED: scan for a clear instruction/task.
GREEN: target check. Decide quickly if the task is really yours, then leave GREEN immediately:
- valid -> click MODE once (go BLUE)
- invalid/unclear -> double-click MODE (go RED)
BLUE: execute the task now with mouse/keyboard. When done, click MODE until it becomes RED.

Rules:
- Use observe only in RED and only when no immediate action is possible.
...
TOOLS = [
    {
        ...
        "function": {
            "name": "observe",
            "description": "Scan only. Use only in RED when no immediate action is possible.",
            "parameters": {
                "type": "object",
                "properties": {
                    "story": {
                        "type": "string",
                        "description": "Memory: Recently... / Now... / Soon... (no mode/mode-box words).",
                    }
                },
                "required": ["story"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what changed + next)."},
                },
                "required": ["x", "y", "story"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "x": {"type": "number", "description": "X coordinate (0-1000)"},
                    "y": {"type": "number", "description": "Y coordinate (0-1000)"},
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what appeared + next)."},
                },
                "required": ["x", "y", "story"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "x": {"type": "number", "description": "X coordinate (0-1000)"},
                    "y": {"type": "number", "description": "Y coordinate (0-1000)"},
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what opened + next)."},
                },
                "required": ["x", "y", "story"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what moved + next)."},
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what you typed + next)."},
                },
                "required": ["text", "story"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "Memory: Recently... / Now... / Soon... (what you revealed + next)."},
                },
                "required": ["dy", "story"],
            },
        },
...
INITIAL_STORY = (
    "Recently: I was idle. Now: I scan the screen for a clear instruction.\n"
    "Soon: validate the task, then execute it quickly."
)
...
# ---- Function prototypes (only what we need beyond the base script)
...
def mouse_drag(x1: int, y1: int, x2: int, y2: int, conv: Coord) -> None:
    ...
    send_input([make_mouse_input(ax1, ay1, MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE), make_mouse_input(0, 0, MOUSEEVENTF_LEFTDOWN)])
    time.sleep(0.05)
...
class HUD:
    ...
    # Mode box state: red=scan, green=validate, blue=execute.
    mode: int = field(default=MODE_SCAN)
    _mode_since: float = field(default_factory=time.time)
    _mode_brush_scan: w.HBRUSH | None = None
    _mode_brush_validate: w.HBRUSH | None = None
    _mode_brush_task: w.HBRUSH | None = None
    ...
    # Keep callback alive to prevent GC issues.
    ...
    def set_mode(self, new_mode: int) -> None:
        """Set the MODE box state and refresh its visuals."""
    ...
    def _cycle_mode(self, delta: int) -> None:
        self.set_mode((self.mode + int(delta)) % 3)
    ...
    def _set_paused_ui(self, paused: bool) -> None:
        ...
        # Editable when paused; read-only when running.
        if self.edit_hwnd:
    ...
    def _wndproc(self, hwnd: w.HWND, msg: int, wparam: w.WPARAM, lparam: w.LPARAM) -> w.LPARAM:
        # Guard: exceptions inside ctypes callbacks are swallowed and look like "nothing happens".
        try:
            if msg == WM_COMMAND:
                cmd_id = _loword(int(wparam))
                notify = (int(wparam) >> 16) & 0xFFFF
                if cmd_id == self._BTN_ID:
                    self._set_paused_ui(not self.paused)
                    return 0
                if cmd_id == self._MODE_ID:
                    if notify == STN_CLICKED:
                        # Click cycles RED -> GREEN -> BLUE -> RED.
                        self._cycle_mode(+1)
                        return 0
                    if notify == STN_DBLCLK:
                        # Double-click is an immediate "reject" path back to RED.
                        self.set_mode(MODE_SCAN)
                        return 0

            elif msg == WM_CTLCOLORSTATIC:
                # Paint MODE box background and text color.
                if self.mode_hwnd and int(lparam) == int(self.mode_hwnd):
                    hdc = w.HDC(wparam)
                    # White text on solid background.
                    WHITE = w.COLORREF(0x00FFFFFF)
                    RED = w.COLORREF(0x000000FF)   # RGB(255,0,0)
                    GREEN = w.COLORREF(0x0000FF00) # RGB(0,255,0)
                    BLUE = w.COLORREF(0x00FF0000)  # RGB(0,0,255)
                    gdi32.SetTextColor(hdc, WHITE)
                    if self.mode == MODE_TASK:
                        gdi32.SetBkColor(hdc, BLUE)
                        return int(self._mode_brush_task) if self._mode_brush_task else 0
                    if self.mode == MODE_VALIDATE:
                        gdi32.SetBkColor(hdc, GREEN)
                        return int(self._mode_brush_validate) if self._mode_brush_validate else 0
                    gdi32.SetBkColor(hdc, RED)
                    return int(self._mode_brush_scan) if self._mode_brush_scan else 0

            elif msg == WM_HSCROLL:
                # For trackbar, lParam is the control HWND.
                ...
                self.stop_event.set()
                # Ensure main thread unblocks if it was waiting.
                self.pause_event.set()
                # Free GDI brushes.
                try:
                    if self._mode_brush_scan:
                        gdi32.DeleteObject(w.HGDIOBJ(self._mode_brush_scan))
                        self._mode_brush_scan = None
                    if self._mode_brush_validate:
                        gdi32.DeleteObject(w.HGDIOBJ(self._mode_brush_validate))
                        self._mode_brush_validate = None
                    if self._mode_brush_task:
                        gdi32.DeleteObject(w.HGDIOBJ(self._mode_brush_task))
                        self._mode_brush_task = None
                except Exception:
                    pass
                return 0

        except Exception as e:
            # Best effort debug signal; avoid raising inside callback.
            try:
    ...
    def _window_thread(self) -> None:
        ...
        # Ensure trackbar class is registered.
        ...
        # Register a window class for our top-level HUD.
        ...
        if not atom:
            # If the class already exists, we can proceed.
        ...
        # Cyan background (COLORREF is 0x00BBGGRR). Cyan RGB (0,255,255) => 0x00FFFF00.
        CYAN = 0x00FFFF00
        ...
        # MODE indicator (clickable). Red=SCAN, Green=VALIDATE, Blue=TASK.
        RED = w.COLORREF(0x000000FF)    # RGB(255,0,0)
        GREEN = w.COLORREF(0x0000FF00)  # RGB(0,255,0)
        BLUE = w.COLORREF(0x00FF0000)   # RGB(0,0,255)
        self._mode_brush_scan = gdi32.CreateSolidBrush(RED)
        self._mode_brush_validate = gdi32.CreateSolidBrush(GREEN)
        self._mode_brush_task = gdi32.CreateSolidBrush(BLUE)
        ...
        # Ensure the label reflects the initial mode.
...
def execute_tool_action(tool: str, args: dict[str, Any], conv: Coord) -> None:
    """Execute a single tool action described by the model."""
    match tool:
        case "click":
...
def main() -> None:
    ...
    print("Starting PAUSED - Click RESUME button to begin\n")
    ...
    with HUD() as hud:
        ...
        # Do not capture / call the model until the user clicks RESUME at least once.
        hud.wait_for_resume()
        ...
        while not hud.stop_event.is_set():
            ...
            if hud.stop_event.is_set():
                break
            # If the model got stuck in GREEN (validate), auto-return to RED.
            if hud.mode == MODE_VALIDATE and (time.time() - hud._mode_since) > VALIDATE_TIMEOUT_SEC:
                hud.set_mode(MODE_SCAN)
            # Do not read HUD text as input; memory exists only in the screenshot.
            time.sleep(0.02)
            step += 1
            ...
            try:
                ...
                if tool != "observe":
                    # Enforce MODE behavior: in RED/GREEN only allow changing the MODE box.
                    if hud.mode != MODE_TASK:
                        allowed = False
                        if tool in ("click", "double_click") and hud.mode_hwnd:
                            # Allow click only if it targets the MODE box.
                            try:
                                rc = w.RECT()
                                if user32.GetWindowRect(hud.mode_hwnd, ctypes.byref(rc)):
                                    px, py = conv.to_screen(float(args2.get("x", 0)), float(args2.get("y", 0)))
                        ...
                        if allowed:
                            ...
                            # Ignore other actions while not in BLUE.
                            time.sleep(0.2)
                    else:
                        execute_tool_action(tool, args2, conv)
                        time.sleep(0.5)

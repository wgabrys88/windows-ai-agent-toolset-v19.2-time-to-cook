...
SYSTEM_PROMPT = """You are FRANZ, a Windows desktop agent. You see one screenshot per step.

Your memory is the cyan FRANZ MEMORY window visible in the image. You write reports there; you read them visually.

Two modes exist:
- OBSERVATION (red button): scan the screen for tasks written for FRANZ; use observe() to write reports.
- EXECUTION (blue button): perform the task using mouse/keyboard tools; use observe() only to confirm results.

Coordinates: 0,0 = top-left; 1000,1000 = bottom-right.

Each tool has a "story" field. Write 100-200 word atemporal reports:
- Recently: what was attempted or set up.
- Now: what is visible or believed true.
- Soon: the next concrete action.

Be action-biased. Decide on one concrete interaction per step."""
...
TOOLS = [
    {
        ...
        "function": {
            "name": "observe",
            "description": "Report current state or search for tasks.",
            "parameters": {
                "type": "object",
                "properties": {"story": {"type": "string", "description": "Status report."}},
                "required": ["story"],
            },
        },
    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "Status report."},
                },
                "required": ["x", "y", "story"],
            },
        },
    ...
    },
    {
        ...
        "function": {
            "name": "double_click",
            "description": "Double-click to activate item.",
            "parameters": {
                "type": "object",
                "properties": {
                    "x": {"type": "number"},
    ...
    },
    {
        ...
        "function": {
            "name": "drag",
            "description": "Click-drag from start to end.",
            "parameters": {
                "type": "object",
                "properties": {
                    "x1": {"type": "number"},
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "dy": {"type": "number", "description": "Scroll amount"},
                    "story": {"type": "string"},
                },
                "required": ["dy", "story"],
            },
...
def test_tool(tool: str, **kwargs) -> None:
    ...
    args = {**kwargs, "story": f"Testing {tool} tool."}

    with HUD() as hud:
        hud.update(args["story"])

...
SYSTEM_PROMPT = """You are FRANZ, a Windows desktop agent. You control the mouse and keyboard using tools.

Memory:
The window titled "FRANZ MEMORY" is your only memory. Keep it short and practical.

Priority:
Prefer ACTION over description. On most turns you must choose an ACTION tool (click/right_click/double_click/drag/type/scroll).
Use observe only when you truly need to locate something first or after you finish a short action sequence.

When you write story (memory):
2-4 short sentences: (1) goal, (2) what you just did, (3) what you will do next.
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "story": {
                        "type": "string",
                        "description": "Memory update only: 2-4 short sentences (goal + next action).",
                    }
                },
                "required": ["story"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "description": "After clicking, update memory: goal + what changed + next action."},
                },
                "required": ["x", "y", "story"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "x": {"type": "number", "description": "X coordinate (0-1000)"},
                    "y": {"type": "number", "description": "Y coordinate (0-1000)"},
                    "story": {"type": "string", "description": "After right-click, update memory: what menu you opened + next action."},
                },
                "required": ["x", "y", "story"],
            },
        },
...
class HUD:
    ...
    def _layout_controls(self) -> None:
        ...
        # Button
        if self.button_hwnd:
            user32.MoveWindow(self.button_hwnd, margin, margin + edit_h + margin, 150, btn_h, True)
        ...
        if self.slider_hwnd:
            user32.MoveWindow(
                self.slider_hwnd,
                margin,
                margin + edit_h + margin + btn_h + margin,
...
def main() -> None:
    ...
    with HUD() as hud:
        ...
        while not hud.stop_event.is_set():
            ...
            # Allow user edits made while paused to become the new story, and ensure the HUD reflects it
            # before we capture the screen for the next request.
            current_story = hud.get_text() or current_story
            hud.update(current_story)
            time.sleep(0.05)

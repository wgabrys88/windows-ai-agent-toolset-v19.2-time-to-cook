...
This file is based on the *stable* runtime loop from working-code.py and adds only
the intended new HUD features from not-working-code-new-features.py:

New HUD features:
- A draggable/resizable topmost "FRANZ MEMORY" window (with title bar)
- PAUSE/RESUME button that gates the agent loop
- Opacity slider (layered window alpha)
- Story is editable while paused; read-only while running

Notes:
- Requires Windows (Win32 APIs via ctypes).
- Uses RichEdit (RICHEDIT50W) so EM_SETBKGNDCOLOR works reliably.
"""
...
user32.PeekMessageW.argtypes = [ctypes.POINTER(MSG), w.HWND, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint]
...
class HUD:
        ...
        try:
            if msg == WM_COMMAND:
                ...
                if cmd_id == self._BTN_ID:
                    ...
                    return 0
                if cmd_id == self._MODE_ID and notify == STN_CLICKED:
                    self.task_mode = not self.task_mode
                    if self.mode_hwnd:
                        user32.SetWindowTextW(self.mode_hwnd, "TASK EXECUTION MODE" if self.task_mode else "SCAN MODE")
                ...
                if self.mode_hwnd and int(lparam) == int(self.mode_hwnd):
                    ...
                    gdi32.SetBkColor(hdc, BLUE if self.task_mode else RED)
                    # Return the brush to use for background.
                    if self.task_mode:
    ...
    def _window_thread(self) -> None:
        ...
        # MODE indicator (clickable). Red = SCAN, Blue = TASK.
        RED = w.COLORREF(0x000000FF)   # RGB(255,0,0)
        BLUE = w.COLORREF(0x00FF0000)  # RGB(0,0,255)
    ...
    def get_text(self) -> str:
        ...
        user32.GetWindowTextW(self.edit_hwnd, buf, length + 1)
        return buf.value
    ...
    with HUD() as hud:
        ...
        while not hud.stop_event.is_set():
            ...
            try:
                ...
                if tool != "observe":
                    # Enforce MODE behavior: in SCAN MODE only allow clicking the MODE box.
                    if not hud.task_mode:
                        allowed = False
                        if tool == "click" and hud.mode_hwnd:
                        ...
                        if allowed:
                            ...
                            # Ignore other actions while in SCAN MODE.
                            time.sleep(0.2)
                    else:
                        execute_tool_action(tool, args2, conv)
                        time.sleep(0.5)
